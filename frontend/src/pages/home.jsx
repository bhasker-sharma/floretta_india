...
const Home = () => {
    ...
    fetch(API_ENDPOINTS.HOMEPAGE)
      ...
      .then((data) => {
        ...
        if (bestsellers.length > 0) {
          ...
          const firstImage =
            bestsellers[0].all_images?.[0]?.url || bestsellers[0].image;
  ...
  const thumbnails = [
    ...
    ...(featuredProduct?.extra_images || []).slice(0, 3),
  ].filter(Boolean);
  ...
  const handleShopNow = () => {
    if (featuredProduct) {
      navigate(getProductUrl(featuredProduct.id, featuredProduct.name));
    }
  };
  ...
  return (
    <>
      <Navbar />
      <Slider
        fetchUrl={API_ENDPOINTS.SLIDERS_BY_PAGE("home")}
        interval={4000}
      />
      ...
      <div className="homep-product-list">
        ...
        <div className="product-grid">
          {homeProducts.map((item) => (
            <div
              key={item.id}
              onClick={() => navigate(getProductUrl(item.id, item.name))}
        ...
        {!featuredProduct ? (
          ...
          <div className="pdp-product-card">
            <div
              ...
              onClick={() =>
                navigate(
                  getProductUrl(featuredProduct.id, featuredProduct.name)
                )
              }
              style={{ cursor: "pointer" }}
            >
            ...
            <div className="pdp-info-section">
              ...
              >
                ...
                <div className="pdp-product-prices">
                  ...
                  <span className="pdp-product-old-price">
                    ₹{featuredProduct.original_price}
                  </span>
                </div>
                <div className="pdp-product-size">
                  <label>SIZE</label>
                  <div className="pdp-size-box">
                    {featuredProduct.volume_ml}ml
                  </div>
                </div>
              </div>
